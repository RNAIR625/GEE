...
@rules_bp.route('/bulk_delete_rules', methods=['DELETE'])
def bulk_delete_rules():
    ...
    try:
        ...
        if not rule_ids:
            return jsonify({'success': False, 'message': 'No rule IDs provided'})
        ...
        for rule_id in rule_ids:
            try:
                # Check if rule exists
                existing_rule = query_db('SELECT RULE_NAME FROM GEE_RULES WHERE RULE_ID = ?', (rule_id,), one=True)
        ...
        logging.error(f"Bulk delete rules failed: {str(e)}")
